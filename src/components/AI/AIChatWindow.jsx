import { X, Send, Download, Bot, Loader2, Settings, ChevronLeft, Trash2 } from "lucide-preact";
...
export function AIChatWindow({
  ...
  const handleClearAICache = async () => {
    if (confirm("Are you sure you want to delete the AI model cache? You will need to re-download the models to use Local AI again.")) {
      try {
        const cacheNames = await caches.keys();
        for (const name of cacheNames) {
          if (name.includes('transformers-cache')) {
    ...
    >
      ...
      <div className="ai-chat-header">
        <div className="ai-chat-header-info">
          {showSettings ? (
            <button className="ai-back-button" onClick={() => setShowSettings(false)}>
              <ChevronLeft size={20} />
            </button>
          ) : (
            <div
          ...
          <h3 className="ai-chat-header-title">
            {showSettings ? "AI Settings" : "Pocket Assistant"}
          </h3>
        </div>
        <div style={{ display: "flex", gap: "8px" }}>
          {!showSettings && (
            <button className="ai-settings-button" onClick={() => setShowSettings(true)}>
              <Settings size={20} />
            </button>
          )}
            ...
            <div className="ai-setting-item">
              ...
              <div className="ai-toggle-group">
                <button 
                  ...
                  onClick={() => setApiConfig({...apiConfig, useRemote: false})}
                >
                  Local AI
                </button>
                <button 
                  className={`ai-toggle-btn ${apiConfig.useRemote ? 'active' : ''}`}
                  onClick={() => setApiConfig({...apiConfig, useRemote: true})}
                >
                  Remote API
                </button>
              </div>
              ...
              <>
                <div className="ai-setting-item">
                  ...
                  <input 
                    ...
                    value={apiConfig.url}
                    onInput={(e) => setApiConfig({...apiConfig, url: e.target.value})}
                ...
                <div className="ai-setting-item">
                  ...
                  <input 
                    ...
                    className="ai-setting-input"
                    value={apiConfig.key}
                    onInput={(e) => setApiConfig({...apiConfig, key: e.target.value})}
                    placeholder="sk-..."
                  />
                </div>
                <div className="ai-setting-item">
                  ...
                  <input 
                    ...
                    onInput={(e) => setApiConfig({...apiConfig, model: e.target.value})}
                    placeholder="gpt-3.5-turbo"
                  />
                </div>
                <p className="ai-settings-tip">
                  Use any OpenAI-compatible API (OpenRouter, Groq, DeepSeek, etc.)
                </p>
              </>
            )}
              ...
              <>
                <div className="ai-setting-item">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <label className="ai-setting-label">Local Model (Hugging Face)</label>
                    <button 
                      className="ai-reset-link"
                      onClick={() => setApiConfig({...apiConfig, localModelId: "onnx-community/LFM2-350M-ONNX"})}
                    >
                      Reset to Default
                    </button>
                  </div>
                  <input 
                    type="text" 
                    className="ai-setting-input"
                    value={apiConfig.localModelId}
                    onInput={(e) => setApiConfig({...apiConfig, localModelId: e.target.value})}
                    placeholder="onnx-community/LFM2-350M-ONNX"
                  />
                  <p className="ai-settings-tip">
                    Default: <code>onnx-community/LFM2-350M-ONNX</code> (Optimized for speed)
                  </p>
                </div>
                <div className="ai-setting-item">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                    <label className="ai-setting-label">Storage Used</label>
                    <span className="ai-storage-value">{storageSize || "Calculating..."}</span>
                  </div>
                  <button 
                ...
                <div className="ai-local-info">
                  <Bot size={24} />
                  <p>Local AI runs entirely on your device using WebGPU/WASM. It's private but resource-heavy.</p>
                </div>
              </>
            )}
          </div>
          ...
          <div className="ai-setup-view">
            ...
            {status === "downloading" ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', width: '100%' }}>
                ...
                <button className="ai-secondary-button" onClick={() => setShowSettings(true)}>
              ...
              <div key={i} className={`chat-message ${msg.role}`}>
                ...
                {msg.role === "ai" && msg.duration && (
                  <div className="ai-message-duration">
                    {apiConfig.useRemote ? 'Generated via Cloud' : 'Generated via Device'} in {msg.duration}
                  </div>
                )}
              </div>
            ))}
